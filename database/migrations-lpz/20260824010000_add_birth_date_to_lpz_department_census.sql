...
--
-- Додає birth_date до lpz_department_census — новий рядок пацієнта на
-- head-cabinet.html (список лікаря, lf-left-bottom) показує дату поступлення/
-- вік/стать/дату народження в одному рядку з ПІБ. Зміна РЕЗУЛЬТАТУ функції
-- (не лише вхідного параметра, як у попередніх p_month-міграціях) — Postgres
-- не дозволяє CREATE OR REPLACE FUNCTION зі зміненим RETURNS TABLE, тому
-- DROP + CREATE, а не REPLACE.
...
CREATE FUNCTION lpz.lpz_department_census(p_org text, p_department uuid, p_doctor uuid DEFAULT NULL::uuid, p_date date DEFAULT CURRENT_DATE)
 RETURNS TABLE(id_case bigint, pib text, age integer, gender text, icd_code text, diagnosis text, hosp_count integer, admission_date date, discharge_date date, days integer, re_admission text, doc_resource_id uuid, birth_date date)
 LANGUAGE sql
 SECURITY DEFINER
