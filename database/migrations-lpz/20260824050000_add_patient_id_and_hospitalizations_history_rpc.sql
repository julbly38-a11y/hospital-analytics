...
--
-- 1) Додає patient_id до lpz_department_census — клік на цифру повторних
--    госпіталізацій (.census-hosp-count, utils.js) потребує стабільного
--    ідентифікатора пацієнта, не ПІБ (колізії).
-- 2) lpz_patient_hospitalizations — уся історія госпіталізацій пацієнта
--    (по всьому org_edrpou, не лише поточному відділенню — той самий
--    рівень довіри, що вже показує повний випадок конкретної
--    госпіталізації, просто в часі).
...
CREATE FUNCTION lpz.lpz_department_census(p_org text, p_department uuid, p_doctor uuid DEFAULT NULL::uuid, p_date date DEFAULT CURRENT_DATE)
 RETURNS TABLE(id_case bigint, pib text, age integer, gender text, icd_code text, diagnosis text, hosp_count integer, admission_date date, discharge_date date, days integer, re_admission text, doc_resource_id uuid, birth_date date, blok text, patient_id uuid)
 LANGUAGE sql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION lpz.lpz_patient_hospitalizations(p_org text, p_patient uuid)
 RETURNS TABLE(id_case bigint, admission_date date, discharge_date date, department_name text, icd_code text, diagnosis text, doc_name text)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'lpz', 'public'
AS $function$
  select h.id_case, h.admission_date, h.discharge_date, h.department_name,
    h.icd_primary as icd_code, coalesce(d.name, h.icd_primary) as diagnosis, h.doc_name
  from lpz.lpz_hospitalizations h
  left join lpz.lpz_icd_diagnoses d on d.code = h.icd_primary
  where h.org_edrpou = p_org and h.patient_id = p_patient
