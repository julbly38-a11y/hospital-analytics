...
export function routeQuery(question, role, empName) {
  ...
  if (role === 'admin' && any(t,'пацієнт','картка','історія хвороб','історія пацієнт')) {
    // Витягуємо ПІБ: слова з великої літери в оригінальному питанні
    const pib = (question.match(/[А-ЯІЇЄҐ][а-яіїєґ'’-]+(?:\s+[А-ЯІЇЄҐ][а-яіїєґ'’-]+){1,2}/g) || [])
      .filter(s => !/^(Покажи|Картка|Історія|Пацієнт|Пацієнта|Інфо|Дані|Хвороби)$/i.test(s))
