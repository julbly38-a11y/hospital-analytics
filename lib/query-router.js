...
export function routeQuery(question, role, empName) {
  ...
  // Картка пацієнта за ПІБ — ТІЛЬКИ для admin (повні персональні дані).
  // ПЕРЕД детектором лікаря: запит явно про пацієнта має пріоритет, навіть якщо
  // прізвище пацієнта збігається з прізвищем лікаря (напр. Ткачук).
  // АЛЕ: відрізняємо "картка пацієнта Ткачук" (шукаю людину) від
  // "Ткачук скільки пацієнтів" (статистика лікаря) — у другому випадку є слова лікаря.
  const looksLikeDoctorStat = any(t,'скільки','пролікував','пролікувала','прийняв','лікар','доктор','летальн','операц','нічн','чергуван','вихідн','діб')
  const explicitPatientCard = any(t,'картка','історія хвороб','історія пацієнт') ||
    /пацієнт[аи]?(\s+|$)[А-ЯІЇЄҐ]/u.test(question)
  if (role === 'admin' && explicitPatientCard && !looksLikeDoctorStat) {
    const stop = /^(Покажи|Картка|Картку|Історія|Історію|Пацієнт|Пацієнта|Пацієнтів|Інфо|Дані|Хвороби|Хвороб|Знайди|Знайти)$/i
    const words = (question.match(/[А-ЯІЇЄҐ][а-яіїєґ'’-]+/g) || []).filter(s => !stop.test(s))
    if (words.length >= 1) {
      const pib = words.join(' ')
      const safe = pib.replace(/'/g, "''")
      if (words.length === 1) {
        return {cached:true, explanation:`Пацієнти з прізвищем «${pib}» — оберіть, написавши повне ПІБ`,
          sql:`SELECT p.full_name as пацієнт,p.age as вік,p.gender as стать,p.district as район,p.locality as населений_пункт,(SELECT COUNT(*) FROM lsmd l WHERE l.patient_id=p.patient_id) as госпіталізацій FROM patients_best p WHERE p.full_name ILIKE '${safe} %' OR p.full_name ILIKE '${safe}' ORDER BY p.full_name LIMIT 100`}
      }
      return {cached:true, explanation:`Картка пацієнта: ${pib}`,
        sql:`SELECT l.admission_date_d as дата,l.icd_primary as мкх,l.admission_department as відділення,l.doc_name as лікар,l.discharge_status as вислід,l.length_of_stay as ліжкодень,p.full_name as пацієнт,p.age as вік,p.gender as стать,p.phone_num as телефон,p.address as адреса,p.district as район FROM patients_best p JOIN lsmd l ON l.patient_id=p.patient_id WHERE p.full_name ILIKE '%${safe}%' ORDER BY l.admission_date_d DESC LIMIT 100`}
    }
  }
  ...
  // Картка пацієнта (стара позиція видалена — перенесено вище)
  // Картка пацієнта (стара позиція видалена — перенесено вище)
  ...
  // Лікарі цілого напрямку: "скільки лікарів хірургічного напрямку" (весь блок)
  if ((t.includes('напрям')||t.includes('блок')) && (t.includes('хірург')||t.includes('терапевт')||t.includes('інтенсивн')||t.includes('параклін')) && any(t,'лікар','доктор','персонал','штат')) {
    let bn = null
    if (t.includes('хірург')) bn = 'хірургічний'
    else if (t.includes('терапевт')) bn = 'терапевтичний'
    else if (t.includes('інтенсивн')) bn = 'інтенсивна_терапія'
    else if (t.includes('параклін')) bn = 'параклінічний'
    if (bn) {
      const clin = `(e.is_clinical=true OR e.position ILIKE '%лікар%' OR e.position ILIKE '%ординатор%' OR e.position ILIKE '%завідувач%')`
      if (has(t,'скільки'))
        return {cached:true,explanation:`Кількість лікарів напрямку: ${bn}`,
          sql:`SELECT COUNT(*) as лікарів FROM empl e JOIN departments d ON d.dept_name=e.department JOIN clinical_blocks b ON b.id=d.block_id WHERE b.name='${bn}' AND ${clin}`}
      return {cached:true,explanation:`Лікарі напрямку: ${bn}`,
        sql:`SELECT e.emp_name as лікар,e.specialization as спеціалізація,e.department as відділення,e.position as посада FROM empl e JOIN departments d ON d.dept_name=e.department JOIN clinical_blocks b ON b.id=d.block_id WHERE b.name='${bn}' AND ${clin} ORDER BY e.department, e.emp_name`}
    }
  }
  ...
  for (const sp of specs)
    if (has(t,sp)&&any(t,'лікар','доктор','список','скільки','всі','покажи')) {
      // Травматолог: ловимо і "травматолог дитячий"; решта — точний матч
      const cond = (sp==='травматолог') ? `specialization LIKE 'травматолог%'` : `specialization = '${sp}'`
