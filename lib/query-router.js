...
export function routeQuery(question, role, empName) {
  ...
  if ((has(t,'відділ')||has(t,'скільки')) && (t.includes('хірург')||t.includes('терапевт')||t.includes('інтенсивн')||t.includes('параклін')) && (t.includes('напрям')||t.includes('блок')||t.includes('хірургічного')||t.includes('терапевтичного'))) {
    let blockName = null
  ...
  for (const [kw,dept] of deptMap) {
    ...
    if (any(t,'лікар','персонал','хто працює','співробітник','штат','завідувач'))
      return {cached:true,explanation:`Лікарі: ${dept}`,sql:`SELECT emp_name as лікар,specialization as спеціалізація,position as посада FROM empl WHERE department ILIKE '%${kw}%' AND (is_clinical=true OR position ILIKE '%лікар%' OR position ILIKE '%ординатор%' OR position ILIKE '%завідувач%') ORDER BY (position ILIKE '%завідувач%') DESC, emp_name`}
    if (any(t,'топ','діагноз','найчастіш','поширен'))
