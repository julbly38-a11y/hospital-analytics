...
function doctorSQL(t, doc) {
  ...
  if (dgx) {
    return {explanation:`Мої випадки (${dgx.label})${pl}`,
      sql:`SELECT admission_date_d as дата_надходження,icd_primary as мкх,admission_department as відділення,discharge_status as вислід,length_of_stay as ліжкодень FROM lsmd WHERE doc_name ILIKE '%${doc}%' AND icd_primary LIKE '${dgx.icd}%' ${df} ORDER BY admission_date_d DESC LIMIT 200`}
  }
...
function doctorDiagnosis(t) {
  const DIAGS = [
    [[' інсульт',' ішемічн'],'I63','ішемічний інсульт'],
