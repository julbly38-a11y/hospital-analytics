...
існувала в схемі, але жодного коду її не читало. Звіряє **лише перші 3
символи** коду (`LEFT(p_icd, 3) BETWEEN code_from AND code_to`) — так само,
як оригінальна `pages/api/stats.js:ICD_BLOCK_CASE`, бо `code_from`/`code_to`
самі рівно 3-символьні (напр. `'G00'`/`'G09'`). Перша версія цієї функції
звіряла повний код (з крапкою, напр. `K86.1`) — 7.2% госпіталізацій
класифікувались у неправильний блок; виправлено в
`20260726110800_fix_icd_block_name_left3_match.sql`.
