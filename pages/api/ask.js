...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r2 = await fetch(`${(process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL)}/rest/v1/rpc/execute_sql_safe`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({ sql_query: safeSql, p_role: role, p_doc_name: empName || null })
    })
