...
function doctorSQL(t, doc) {
  ...
  if (any(t,'спеціалізац','ким працює','яка посада','хто такий'))
    return {explanation:`Інфо: ${doc}`,
      sql:`SELECT emp_name as лікар,specialization as спеціалізація,position as посада,department as відділення FROM empl WHERE emp_name ILIKE '%${doc}%'`}

  if (has(t,'вихідн','діб'))
    return {explanation:`Вихідні доби: ${doc}${pl}`,
      sql:`SELECT doc_name as лікар,COUNT(DISTINCT admission_date_d) AS вихідних_діб FROM lsmd WHERE doc_name ILIKE '%${doc}%' AND EXTRACT(DOW FROM admission_date_d) IN (0,6) ${df} GROUP BY doc_name`}

  if (has(t,'нічн'))
    return {explanation:`Нічні: ${doc}${pl}`,
      sql:`SELECT doc_name as лікар,COUNT(*) as нічніх FROM lsmd WHERE doc_name ILIKE '%${doc}%' AND admission_ts IS NOT NULL AND (EXTRACT(HOUR FROM admission_ts::timestamp)>=22 OR EXTRACT(HOUR FROM admission_ts::timestamp)<6) ${df} GROUP BY doc_name`}

  if (any(t,'летальн','смертн','померл'))
    return {explanation:`Летальність: ${doc}${pl}`,
      sql:`SELECT doc_name as лікар,COUNT(*) as всього,SUM(CASE WHEN discharge_status='Помер' THEN 1 ELSE 0 END) as померло FROM lsmd WHERE doc_name ILIKE '%${doc}%' ${df} GROUP BY doc_name`}

  if (any(t,'операц','хірург'))
    return {explanation:`Операції: ${doc}${pl}`,
      sql:`SELECT doc_name as лікар,COUNT(*) as всього,SUM(CASE WHEN operation_id IS NOT NULL THEN 1 ELSE 0 END) as операцій FROM lsmd WHERE doc_name ILIKE '%${doc}%' ${df} GROUP BY doc_name`}

  return {explanation:`Статистика: ${doc}${pl}`,
    sql:`SELECT doc_name as лікар,COUNT(*) as випадків,COUNT(DISTINCT patient_id) as пацієнтів,ROUND(AVG(length_of_stay),1) as ліжкодень,SUM(CASE WHEN discharge_status='Помер' THEN 1 ELSE 0 END) as померло FROM lsmd WHERE doc_name ILIKE '%${doc}%' ${df} GROUP BY doc_name`}
}
...
export function routeQuery(question) {
  ...
  const depts=[[[' гастро'],'гастроентерол'],[[' гематол'],'гематолог'],[[' нейрохірург'],'нейрохірург'],
    [[' опіков'],'опіков'],[[' терапевт','№1'],'терапевт'],[[' терапевт','№2'],'терапевт'],
    [[' травматол','діт'],'травматол'],[[' травматол','дорос'],'травматол'],
    [[' урологічн'],'урологічн'],[[' хірургічн','№1'],'хірургічн'],[[' хірургічн','№2'],'хірургічн'],
    [[' невролог'],'невролог'],[[' анестез'],'анестез'],[[' реанімац'],'анестез']]
  for (const [kws,like] of depts)
    if (kws.every(k=>t.includes(k.trim())))
      return {cached:true,explanation:`Відділення: ${like}`,
        sql:`SELECT department as відділення,total_cases as випадків,unique_patients as пацієнтів,avg_bed_days as ліжкодень,deaths as померло,death_rate_pct as летальність,operations as операцій FROM v_department_stats WHERE department ILIKE '%${like}%'`}
  ...
  const specs=['хірург','терапевт','уролог','кардіолог','невропатолог','нейрохірург','анестезіолог',
    'гастроентеролог','гінеколог','ендокринолог','травматолог','онколог','педіатр',
    'пульмонолог','офтальмолог','нефролог','гематолог','інфекціоніст']
  ...
  if (has(t,'діагноз','відділ')||has(t,'захворюван','відділ'))
    return {cached:true,explanation:'Захворюваність по відділеннях',sql:`SELECT department as відділення,disease_category as категорія,cases as випадків,percent_of_dept as відс FROM v_morbidity_by_department WHERE department IS NOT NULL ORDER BY department,cases DESC`}

  const diags=[[[' панкреатит'],'K86','панкреатит'],[[' інсульт'],'I63','інсульт'],
    [[' геморагічн'],'I61','геморагічний інсульт'],[[' інфаркт',' міокард'],'I21','інфаркт'],
    [[' апендицит'],'K35','апендицит'],[[' холецистит'],'K80','холецистит'],
    [[' цироз'],'K74','цироз'],[[' діабет'],'E11','діабет'],[[' пневмон'],'J18','пневмонія'],
    [[' гіпертонічн'],'I11','гіпертонія'],[[' струс'],'S06','струс'],
    [[' тромбофлебіт'],'I80','тромбофлебіт'],[[' простат'],'N40','простата']]
  for (const [kws,icd,label] of diags)
    if (kws.every(k=>t.includes(k.trim())))
      return {cached:true,explanation:`Діагноз: ${label}`,
        sql:`SELECT d.icd_primary as код,i.diagnosis_level3 as діагноз,d.cases as випадків,d.unique_patients as пацієнтів,d.avg_bed_days as ліжкодень,d.deaths as померло FROM v_diagnosis_stats d LEFT JOIN icd_10 i ON d.icd_primary=i.icd_code WHERE d.icd_primary LIKE '${icd}%' ORDER BY d.cases DESC LIMIT 20`}

  if (has(t,'годин')&&any(t,'пік','навантаж','по','розподіл'))
